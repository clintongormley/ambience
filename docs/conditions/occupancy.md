...
your other rules. See [Precedence](#precedence-why-occupancy-sits-below-entity-state)
below.
...
- **Occupied / Vacant** — whether the chosen sensors should be active
  (occupied) or clear (vacant).
...
A sensor reporting `unavailable` or `unknown` never counts as occupied or
vacant — it is treated as unobservable, so the condition does not pass on it.
