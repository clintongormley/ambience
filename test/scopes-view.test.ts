...
describe("ambience-scopes-view", () => {
  ...
  test("scope row shows an error-severity problem flag when a scene references a missing entity", async () => {
    ...
    const flag = header.querySelector("ambience-problem-flag");
  ...
  test("scope row shows no problem flag when all scenes are clean", async () => {
    ...
    const header = el.shadowRoot.querySelector(".scope-row.house .scope-header");
    expect(header.querySelector("ambience-problem-flag")).toBeFalsy();
  });

  test("tapping the scope problem flag reveals the detail", async () => {
    const broken: Scene = {
      name: "broken",
      category: "c1",
      when: {},
    ...
    const flag = el.shadowRoot.querySelector(
      ".scope-row.house .scope-header ambience-problem-flag",
    );
    (flag.shadowRoot.querySelector(".problem-flag") as HTMLElement).click();
    await flag.updateComplete;
    expect(flag.shadowRoot.querySelector(".details")).toBeTruthy();
  });
  ...
  test("scope kebab no longer offers an Auto-triggers item", async () => {
    el = await mount();
    const header = el.shadowRoot.querySelector("li.scope-row.house .scope-header") as HTMLElement;
    const kebab: any = header.querySelector("ambience-kebab-menu");
    (kebab.shadowRoot.querySelector(".kebab-trigger") as HTMLButtonElement).click();
    await kebab.updateComplete;
    expect(kebab.shadowRoot.querySelector("[data-action='auto']")).toBeNull();
    expect(kebab.shadowRoot.querySelector("[data-action='run']")).toBeTruthy();
  });

  test("show-auto-triggers from a category opens the modal for that category", async () => {
    el = await mount({
      areaConfigs: {
        living_room: {
          scenes: [
            { name: "A", category: "lighting", when: {}, actions: [] },
            { name: "B", category: "blinds", when: {}, actions: [] },
          ],
        },
      },
    });
    ...
    expect(modal.category).toBe("lighting");
    expect(modal.scope).toEqual({ kind: "area", id: "living_room" });
    // Scenes are filtered to the category (only "A" is lighting).
