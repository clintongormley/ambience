...
describe("ambience-auto-triggers-modal", () => {
  ...
  test("entity rows show the entity id and a domain-derived icon", async () => {
    el = await mount({
      open: true,
      triggers: [{ key: "entity:light.kitchen", kind: "entity", entity_id: "light.kitchen", enabled: true }],
    });
  ...
  test("group rows render a representative icon", async () => {
    el = await mount({ open: true });
    const sun = el.shadowRoot.querySelector("li[data-test='trigger-ro-group:sun'] ha-icon.row-icon");
    expect(sun?.getAttribute("icon")).toBe("mdi:weather-sunny");
    const time = el.shadowRoot.querySelector("li[data-test='trigger-ro-group:time'] ha-icon.row-icon");
  ...
  test("the Sun row opens more-info for sun.sun when that entity exists", async () => {
    el = await mount({
      ...
      triggers: [{ key: "group:sun", kind: "sun", suns: [{ anchor: "sunset", offset: 0 }], enabled: true }],
    });
    let detail: any;
    el.addEventListener("hass-more-info", (e: any) => (detail = e.detail));
