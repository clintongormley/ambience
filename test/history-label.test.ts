...
function label(action: string, scene: string | null, scope: string): string {
  const sceneName = scene && scene.trim() ? scene : localize(undefined, "ui.history_untitled", "Untitled");
...
describe("AmbienceScopesView._historyLabel", () => {
  function makeView() {
    ...
    el.hass = {};
    el._store = { areas: [{ area_id: "lr", name: "Living Room" }], floors: [] };
    return el;
  }
  it("resolves an area name", () => {
    const el = makeView();
    expect(
      el._historyLabel({ action: "delete", scene_name: "Movie night", scope_kind: "area", scope_id: "lr" }),
