...
describe("summariseAction", () => {
  ...
  test("legacy entity_ids action still summarises as before", () => {
    const action: ActionSpec = {
      service: "light.turn_on",
      entity_ids: ["light.a", "light.b"],
      params: {},
  ...
  test("target-only area_id action renders area name with '(area)' suffix", () => {
    const hass = {
      localize: () => undefined,
    ...
    expect(summariseAction(action, { hass, exposedActions })).toBe("Set light: Kitchen (area)");
  });

  test("target-only label_id action renders label name with '(label)' suffix", () => {
    const hass = {
      localize: () => undefined,
    ...
    const action: ActionSpec = {
      service: "light.turn_on",
      target: { label_id: ["reading"] },
      params: {},
    };
    expect(summariseAction(action, { hass, exposedActions })).toBe("Set light: reading (label)");
  });

  test("target-only device_id action renders device name with '(device)' suffix", () => {
    const hass = {
      localize: () => undefined,
      devices: { abc123: { name: "Bedroom Hub" } },
    };
    const action: ActionSpec = {
      service: "light.turn_on",
    ...
    expect(summariseAction(action, { hass, exposedActions })).toBe(
      "Set light: Bedroom Hub (device)",
    );
  });

  test("mixed entity_id + area_id target renders both parts", () => {
    const hass = {
      localize: () => undefined,
      areas: { kitchen: { name: "Kitchen" } },
    };
    ...
    expect(summariseAction(action, { hass, exposedActions })).toBe(
      "Set light: 2 lights, Kitchen (area)",
    );
  });

  test("area_id falls back to raw id when area not in hass.areas", () => {
    const hass = { localize: () => undefined };
    const action: ActionSpec = {
      service: "light.turn_on",
      target: { area_id: ["unknown_area"] },
      params: {},
    };
    expect(summariseAction(action, { hass, exposedActions })).toBe(
      "Set light: unknown_area (area)",
    );
  });
});
