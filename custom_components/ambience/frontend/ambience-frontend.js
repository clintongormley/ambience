...
`;function je(t,n){return l`<span class="category-swatch" style=${Pt(t)}>
  ...
  </span>`}function J(t){return t.kind==="house"?"house":`${t.kind}:${t.id}`}function Li(t,n){if(!n||n.entity==null)return[...t];let e=Array.isArray(n.entity)?n.entity:[n.entity];if(e.length===0)return[...t];let r=new Set,i=!1;for(let s of e){if(!s||typeof s!="object")continue;let o=s.domain;if(o==null){i=!0;continue}if(Array.isArray(o))for(let a of o)typeof a=="string"&&r.add(a);else typeof o=="string"&&r.add(o)}return i||r.size===0?[...t]:t.filter(s=>{let o=s.indexOf(".");return o<0?!1:r.has(s.slice(0,o))})}function Ot(t,n,e=[]){let r=t;if(!r?.entities)return[];let i=r.entities,s=r.devices??{},o=r.areas??{},a=n.kind==="area"?new Set([n.id]):n.kind==="floor"?new Set(Object.values(o).filter(h=>h.floor_id===n.id).map(h=>h.area_id)):null,c=h=>{let m=h.area_id??(h.device_id?s[h.device_id]?.area_id??null:null);return m==null?!1:a===null?!0:a.has(m)};return Object.values(i).filter(c).filter(h=>e.length===0||e.includes(h.entity_id.split(".")[0])).map(h=>h.entity_id).sort()}function st(t){let{priority:n,pinned:e,shadowed_by:r,...i}=t;return i}var se=class extends y{constructor(){super(...arguments);this.items=[];this._open=!1;this._onKeydown=e=>{e.key==="Escape"&&this._open&&(this._open=!1)}}_triggerLabel(){return this.label??d(this.hass,"ui.more_actions","More actions")}_select(e,r){r.stopPropagation(),this._open=!1,this.dispatchEvent(new CustomEvent("menu-action",{detail:{id:e},bubbles:!0,composed:!0}))}_renderItems(){return this.items.map(e=>l`
...
`;function _l(t){return t.kind==="entity"?`${t.entity_id} ${t.old} \u2192 ${t.new}`:t.detail?`${me(t.kind)} ${t.detail}`:me(t.kind)}function yl(t,n,e){let r=Object.entries(t.params??{}).filter(([,s])=>s!=null&&s!=="").map(([s,o])=>`${at(s,t.service,e)}: ${ge(n,o)}`).join(", "),i=rt(t.service);return r?`${i} \xB7 ${r}`:i}function bl(t){return t.reduce((n,e)=>n+(e.entity_ids?.length??0),0)}function $l(t,n){let e=t.index+1;return t.disabled?l`<div class="rule disabled">Rule #${e} ${t.name??"\u2014"}: disabled</div>`:t.evaluated?l`
  ...
  `}connectedCallback(){super.connectedCallback(),pe(this)}render(){return l`
      <header>
        <h1>
          ${si({dark:!!this.hass.themes?.darkMode,title:d(this.hass,"ui.panel_title","Ambience")})}
        </h1>
        <button
          class="settings-btn"
