...
A condition you leave unset, or do not add at all, is treated as a **wildcard**:
it always passes. A scene with no conditions whatsoever matches unconditionally,
whenever Ambience evaluates that area.
...
once the **condition's own test** has been continuously true for at least that
long.
