...
`;function Kl(t){return t.kind==="entity"?`${t.entity_id} ${t.old} \u2192 ${t.new}`:t.detail?`${fe(t.kind)} ${t.detail}`:fe(t.kind)}function Vl(t,n,e){let r=Object.entries(t.params??{}).filter(([,s])=>s!=null&&s!=="").map(([s,a])=>`${mt(s,t.service,e)}: ${ve(n,a)}`).join(", "),i=ot(t.service);return r?`${i} \xB7 ${r}`:i}function Gl(t){return t.reduce((n,e)=>n+(e.entity_ids?.length??0),0)}function Yl(t,n){let e=t.index+1;return t.disabled?l`<div class="scene disabled">Scene #${e} ${t.name??"\u2014"}: disabled</div>`:t.evaluated?l`
  ...
  `}var I=class extends b{constructor(){super(...arguments);this.category="";this.categoryName=null;this.open=!1;this._records=[];this._schemas={};this._expanded=new Set;this._loading=!0;this._error="";this._hasNew=!1}connectedCallback(){super.connectedCallback(),this._poll=setInterval(()=>this._checkNew(),5e3)}disconnectedCallback(){super.disconnectedCallback(),this._poll&&clearInterval(this._poll)}updated(e){this.open&&(e.has("open")||e.has("category")||e.has("scope"))&&this._load()}_mine(e){return e.filter(r=>r.scope_kind===this.scope.scope_kind&&r.scope_id===this.scope.scope_id&&r.category===this.category)}async _load(){this._error="",this._loading=!0,this._hasNew=!1,this._expanded=new Set;try{let e=await $r(this.hass);if(!this.isConnected)return;this._records=this._mine(e),this._loading=!1,this._loadSchemas()}catch(e){this._error=e.message||String(e),this._loading=!1}}async _loadSchemas(){let e=[...new Set(this._records.flatMap(s=>s.actions.map(a=>a.service)))].filter(s=>!(s in this._schemas));if(e.length===0)return;let r=await Promise.all(e.map(async s=>{try{return[s,await _e(this.hass,s)]}catch{return null}}));if(!this.isConnected)return;let i={...this._schemas};for(let s of r)s&&(i[s[0]]=s[1]);this._schemas=i}async _checkNew(){if(!(!this.open||!this.isConnected||document.visibilityState!=="visible"))try{let r=this._mine(await $r(this.hass))[0]?.timestamp??null,i=this._records[0]?.timestamp??null;r&&(!i||r>i)&&(this._hasNew=!0)}catch{}}_toggle(e){let r=new Set(this._expanded);r.has(e)?r.delete(e):r.add(e),this._expanded=r}_onClose(){this.dispatchEvent(new CustomEvent("close",{bubbles:!0,composed:!0}))}render(){if(!this.open)return E;let e=this.categoryName??this.category;return l`
    ...
    `}_defaultCategoryId(){return this._filterCategory!==""?this._filterCategory:[...this._categories].sort((r,i)=>r.name.localeCompare(i.name))[0]?.id??""}get _editingScene(){return this._editing?this._editing.seed?this._editing.seed:this._editing.isNew?{when:{},actions:[],category:this._defaultCategoryId()}:this._getConfig(this._editing.scope)?.scenes[this._editing.index]??null:null}get _editorConditions(){return this._editing?this._conditions.slice().sort((e,r)=>r.priority-e.priority):[]}get _takenSceneNames(){let e=new Map,r=this._editing,i=(s,a)=>{if(!a)return;let o=!!r&&!r.isNew&&M(r.scope)===M(s);a.scenes.forEach((c,h)=>{if(o&&h===r.index)return;let f=c.name?.trim().toLowerCase();if(!f)return;let p=Vt(s,c.category),_=e.get(p);_||(_=new Set,e.set(p,_)),_.add(f)})};i({kind:"house"},this._house);for(let s of this._floors)i({kind:"floor",id:s.floor_id},this._floorConfigs.get(s.floor_id));for(let s of this._areas)i({kind:"area",id:s.area_id},this._areaConfigs.get(s.area_id));return e}get _scopeOptions(){return[{scope:{kind:"house"},label:d(this.hass,"ui.scope_house","House")},...this._floors.map(e=>({scope:{kind:"floor",id:e.floor_id},label:e.name})),...this._areas.map(e=>({scope:{kind:"area",id:e.area_id},label:e.name}))]}_summary(e){if(e.scenes.length===0)return d(this.hass,"ui.not_configured","not configured");let r=this._filterCategory===""?e.scenes.length:e.scenes.filter(s=>s.category===this._filterCategory).length,i=r===1?d(this.hass,"ui.scene_singular","scene"):d(this.hass,"ui.scene_plural","scenes");return`${r} ${i}`}get _conditionsUnconfigured(){let e=!this._weatherConfig||this._weatherConfig.entity==null,r=this._dayConfig,i=!r||r.workday_sensor==null&&r.workday_calendar==null;return e||i}_openSettings(e){this.dispatchEvent(new CustomEvent("ambience-open-settings",{detail:{tab:e},bubbles:!0,composed:!0}))}_dismissConditionsHint(){this._conditionsHintDismissed=!0,td()}_renderBanners(){return this._staticLoaded?this._actions.length===0?l`
      ...
      `:"":""}_orderedScopeRows(){let e=[{scope:{kind:"house"},name:d(this.hass,"ui.scope_house","House"),cfg:this._house,rowClass:"house"}];for(let s of this._floors){let a=this._floorConfigs.get(s.floor_id);a&&e.push({scope:{kind:"floor",id:s.floor_id},name:s.name,cfg:a,rowClass:"floor"})}for(let s of this._areas){let a=this._areaConfigs.get(s.area_id);a&&e.push({scope:{kind:"area",id:s.area_id},name:s.name,cfg:a,rowClass:"area"})}let r=[],i=[];for(let s of e)(this._isSwitchedOff(s.scope)?i:r).push(s);return[...r,...i]}_isSwitchedOff(e){let r=this._switchEntityIds.get(M(e));return r?this.hass.states?.[r]?.state==="off":!1}render(){return l`
      ${this._error?l`<p class="error">${this._error}</p>`:""}
      ${this._renderBanners()}
      ${this._renderFilter()}
      <ul>
        ${Ci(this._orderedScopeRows(),e=>M(e.scope),e=>this._renderScopeRow(e.scope,e.name,e.cfg,e.rowClass))}
