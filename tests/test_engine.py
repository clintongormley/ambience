"""Scene engine is pure: no HA, no I/O. Every `when` key is a condition name."""
...
def test_empty_scene_list_returns_none(conditions: dict[str, FakeCondition]) -> None:
    assert resolve([], {"mode": "movie", "tod": "evening"}, conditions) is None


def test_single_key_match(conditions: dict[str, FakeCondition]) -> None:
    scenes = [{"when": {"mode": "movie"}, "actions": []}]
    result = resolve(scenes, {"mode": "movie"}, conditions)
...
def test_key_mismatch_skips_scene(conditions: dict[str, FakeCondition]) -> None:
    scenes = [{"when": {"mode": "reading"}, "actions": []}]
    assert resolve(scenes, {"mode": "movie"}, conditions) is None


def test_none_predicate_is_wildcard(conditions: dict[str, FakeCondition]) -> None:
    scenes = [{"when": {"mode": None}, "actions": []}]
    assert resolve(scenes, {"mode": "movie"}, conditions) is not None
...
def test_condition_predicate_match(conditions: dict[str, FakeCondition]) -> None:
    scenes = [{"when": {"mode": "movie", "tod": "evening"}, "actions": []}]
    result = resolve(scenes, {"mode": "movie", "tod": "evening"}, conditions)
    assert result is not None
    assert result[0] == 0
...
def test_condition_predicate_mismatch(conditions: dict[str, FakeCondition]) -> None:
    scenes = [{"when": {"mode": "movie", "tod": "evening"}, "actions": []}]
    assert resolve(scenes, {"mode": "movie", "tod": "morning"}, conditions) is None
...
def test_first_match_wins(conditions: dict[str, FakeCondition]) -> None:
    ...
    result = resolve(scenes, {"mode": "movie", "tod": "evening"}, conditions)
    assert result is not None
    assert result[0] == 1  # the wildcard scene comes before the specific evening scene


def test_unknown_condition_skips_scene(conditions: dict[str, FakeCondition]) -> None:
    scenes = [
...
def test_none_snapshot_skips_scene(conditions: dict[str, FakeCondition]) -> None:
    scenes = [
        {"when": {"mode": "movie", "tod": "evening"}, "actions": []},
        {"when": {"mode": "movie"}, "actions": []},
    ]
    result = resolve(scenes, {"mode": "movie", "tod": None}, conditions)
    assert result is not None
    assert result[0] == 1


def test_predicate_none_is_wildcard(conditions: dict[str, FakeCondition]) -> None:
    scenes = [{"when": {"mode": "movie", "tod": None}, "actions": []}]
    result = resolve(scenes, {"mode": "movie", "tod": "anything"}, conditions)
    assert result is not None


def test_missing_condition_skips_constrained_scene(
    conditions: dict[str, FakeCondition],
) -> None:
    # If a condition isn't in the conditions dict, a scene constraining it cannot match.
    no_mode = {k: v for k, v in conditions.items() if k != "mode"}
    scenes = [
        {"when": {"mode": "movie"}, "actions": []},
        {"when": {}, "actions": []},
    ]
    result = resolve(scenes, {"mode": "movie"}, no_mode)
    assert result is not None
    assert result[0] == 1


def test_engine_routes_through_script_condition() -> None:
    """A scene whose `when.script` predicate matches the script snapshot fires."""
    ...
    match = resolve(scenes, {"script": snap}, conditions)
