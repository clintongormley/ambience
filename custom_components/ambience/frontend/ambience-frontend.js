...
`;function Vl(t){return t.kind==="entity"?`${t.entity_id} ${t.old} \u2192 ${t.new}`:t.detail?`${fe(t.kind)} ${t.detail}`:fe(t.kind)}function Kl(t,n,e){let r=Object.entries(t.params??{}).filter(([,s])=>s!=null&&s!=="").map(([s,a])=>`${mt(s,t.service,e)}: ${ve(n,a)}`).join(", "),i=ot(t.service);return r?`${i} \xB7 ${r}`:i}function Gl(t){return t.reduce((n,e)=>n+(e.entity_ids?.length??0),0)}function Yl(t,n){let e=t.index+1;return t.disabled?l`<div class="scene disabled">Scene #${e} ${t.name??"\u2014"}: disabled</div>`:t.evaluated?l`
    ...
    ${t.predicates.map(r=>l`
        <div class="pred ${r.passed?"pass":"fail"}" style="padding-left:1rem">
          ${r.passed?"\u2713":"\u2717"} ${q(n,r.condition_key)}${r.detail?l` <span class="dim">[${ql(n,r.condition_key,r.detail)}]</span>`:S}
        </div>`)}
  `:l`<div class="scene skipped">Scene #${e} ${t.name??"\u2014"}: not evaluated</div>`}function lr(t,n,e,r,i){let s=t.actions.map(c=>ot(c.service)).join(", "),a=Gl(t.actions),o=t.explanation!==null||t.actions.length>0;return l`
    <div class="eval">
      <div class="top">
        <span class="outcome ${t.outcome}">${t.outcome.replace(/_/g," ")}</span>
        <span class="cause">${Vl(t.cause)}</span>
  ...
  `,u([m({attribute:!1})],ke.prototype,"hass",2),u([g()],ke.prototype,"_config",2),u([g()],ke.prototype,"_warnings",2),ke=u([w("ambience-day-config")],ke);var sd=["clear-night","cloudy","fog","hail","lightning","lightning-rainy","partlycloudy","pouring","rainy","snowy","snowy-rainy","sunny","windy","windy-variant","exceptional"],pe=class extends b{constructor(){super(...arguments);this._config={entity:null,groups:[]};this._warnings=[];this._expanded=new Set}async connectedCallback(){super.connectedCallback(),this._config=await Bt(this.hass)}async _persist(){let e=await Ui(this.hass,this._config.entity,this._config.groups);this._warnings=e.warnings??[]}_onEntityChange(e){this._config={...this._config,entity:e.detail.value||null},this._persist()}_nextGroupId(e){let r=new Set(e.map(i=>i.id));for(let i=1;i<=e.length+1;i++){let s=`group_${i}`;if(!r.has(s))return s}return`group_${e.length+1}`}_addGroup(){let e=this._nextGroupId(this._config.groups);this._config={...this._config,groups:[...this._config.groups,{id:e,label:"",conditions:[]}]},this._expanded=new Set([...this._expanded,e]),this._persist()}_toggleExpand(e){let r=new Set(this._expanded);r.has(e)?r.delete(e):r.add(e),this._expanded=r}_updateGroup(e,r){this._config={...this._config,groups:this._config.groups.map((i,s)=>s===e?{...i,...r}:i)},this._persist()}_removeGroup(e){let r=this._config.groups[e];if(this._config={...this._config,groups:this._config.groups.filter((i,s)=>s!==e)},r){let i=new Set(this._expanded);i.delete(r.id),this._expanded=i}this._persist()}_conditionsSchema(){return[{name:"conditions",selector:{select:{multiple:!0,mode:"dropdown",options:sd.map(e=>({value:e,label:We(this.hass,e)}))}}}]}_renderConditions(e,r){if(customElements.get("ha-form"))return l`<ha-form
    ...
    .warnings ul { margin: 0.3rem 0 0 0; padding-left: 1.2rem; }
  `,u([m({attribute:!1})],pe.prototype,"hass",2),u([g()],pe.prototype,"_config",2),u([g()],pe.prototype,"_warnings",2),u([g()],pe.prototype,"_expanded",2),pe=u([w("ambience-weather-config")],pe);var ad=new Set(["time_of_day","day","weather"]),Se=class extends b{constructor(){super(...arguments);this._conditions=[];this._error=""}async connectedCallback(){super.connectedCallback();try{this._conditions=await zt(this.hass)}catch(e){this._error=e.message||String(e)}}render(){let e=this._conditions.filter(r=>ad.has(r.name)).slice().sort((r,i)=>i.priority-r.priority);return l`
      ${this._error?l`<p class="error">${this._error}</p>`:""}
      ${e.map(r=>l`
